...
class Orbit:
    ...
    def __init__(self, x0, fixedValue, mu, jacobi=None, stability=None):
    ...
    def getJacobi(self):
        ...
        self.jacobi = -1/2 * (self.x0[3]**2 + self.x0[4]**2 + self.x0[5]**2) + 2 * (1/2 * (self.x0[0]**2 + self.x0[1]**2)
...
class OrbitContinuation:
    ...
    #--------------------------------------------------------------------------
    @staticmethod
    def pseudoArcLenConti(x0, familyStep, orbitNumber, mu, epsilon):

        # sets the iteratively adjusted initial condition
        x_n = x0
        # calculates T/2 by integrating until y changes sign
        try:
            tau_n = Utility.halfPeriod(x_n, mu, 1.0e-11)
        except ValueError:
            return
        # calculation of halo orbits
        for i in range(orbitNumber):

            outData = NumericalMethods.diffCorrections(x_n, mu, epsilon, tau=tau_n)
            x_n = outData[0]
            tau_n = outData[1]
            phi = outData[2]
            xRef = outData[3]
